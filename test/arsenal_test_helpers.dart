...
Future<void> loadArsenalFonts() async {
  final bigShoulders = FontLoader('packages/arsenal/BigShouldersDisplay');
  for (final v in ['Thin', 'ExtraLight', 'Light', 'Regular', 'Medium', 'SemiBold', 'Bold', 'ExtraBold', 'Black']) {
    bigShoulders.addFont(_fontFile('lib/fonts/BigShouldersDisplay-$v.ttf'));
  }
  await bigShoulders.load();

  final rajdhani = FontLoader('packages/arsenal/Rajdhani');
  for (final v in ['Light', 'Regular', 'Medium', 'SemiBold', 'Bold']) {
    rajdhani.addFont(_fontFile('lib/fonts/Rajdhani-$v.ttf'));
  }
  await rajdhani.load();

  final jetBrains = FontLoader('packages/arsenal/JetBrainsMono');
  for (final v in ['Thin', 'ExtraLight', 'Light', 'Regular', 'Medium', 'SemiBold', 'Bold', 'ExtraBold']) {
    jetBrains.addFont(_fontFile('lib/fonts/JetBrainsMono-$v.ttf'));
  }
  for (final v in [
    'ThinItalic',
    'ExtraLightItalic',
