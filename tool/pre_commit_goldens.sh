...
ALL_TEST_FILES=(
  ...
  "$TEST_DIR/ar_example_operator_profile_test.dart"
  "$TEST_DIR/ar_example_mission_control_test.dart"
)

# Returns the test file for a given component path, or empty string.
component_test_file() {
  case "$1" in
    lib/components/ar_scaffold.dart)    echo "$TEST_DIR/ar_scaffold_test.dart" ;;
    lib/components/ar_button.dart)      echo "$TEST_DIR/ar_button_test.dart" ;;
    lib/components/ar_progress_steps.dart) echo "$TEST_DIR/ar_progress_steps_test.dart" ;;
    lib/components/ar_chip.dart)        echo "$TEST_DIR/ar_chip_test.dart" ;;
    lib/components/ar_avatar.dart)      echo "$TEST_DIR/ar_avatar_test.dart" ;;
    lib/components/ar_tag.dart)         echo "$TEST_DIR/ar_tag_test.dart" ;;
    lib/components/ar_bottom_sheet.dart) echo "$TEST_DIR/ar_scaffold_test.dart" ;;
    lib/components/ar_app_bar.dart)     echo "$TEST_DIR/ar_app_bar_test.dart" ;;
    lib/components/ar_bottom_nav.dart)  echo "$TEST_DIR/ar_bottom_nav_test.dart" ;;
    lib/examples/ar_example_operator_profile.dart) echo "$TEST_DIR/ar_example_operator_profile_test.dart" ;;
    lib/examples/ar_example_mission_control.dart)  echo "$TEST_DIR/ar_example_mission_control_test.dart" ;;
    *) echo "" ;;
  esac
}
...
if ! echo "$staged_files" | grep -qF "lib/"; then
  exit 0
fi
